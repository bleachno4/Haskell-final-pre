...
## 1.1 高频押题：Functor / Maybe / Monad

这一部分建议四个人都背熟。老师如果问 Haskell 基础概念，不要只背定义，按这个模板回答：

```text
概念是什么 -> 代码里哪里用了 -> 为什么适合这个 EDA demo
```

### 押题 1：`Maybe` 在哪里用了？为什么不用异常？

答：

`Maybe a` 表示一个计算可能成功，也可能失败：
...
含义是：解析成功就返回结果和剩余字符串，失败就返回 `Nothing`。  
这里不用异常，是因为解析失败是正常分支，不是程序崩溃。用 `Maybe` 可以把失败写进类型。

补一句更完整的回答：

> 小 parser 用 `Maybe` 表示简单成功/失败；外层 `parseDesign` 用 `Either String Design`，因为外层需要给用户错误信息。

### 押题 2：`Maybe` 和 `Either` 区别是什么？
...
- parser 内部的小步骤用 `Maybe`，因为只需要知道能不能匹配。
- `parseDesign` 和 `eval` 用 `Either String ...`，因为需要告诉用户是 parse error、unknown signal，还是 cyclic signal definition。

一句话：

> `Maybe` 适合简单失败，`Either` 适合需要错误信息的失败。

### 押题 3：Functor 是什么？你们哪里用了？

答：

Functor 的核心是 `fmap`，也就是把普通函数应用到“包在上下文里的值”上：
...
`parseNot` 的类型可以理解成 `Parser Expr`，不是裸的 `Expr`。  
`Not <$> parseNot` 表示：如果解析成功，就把结果变成 `Not expr`；如果解析失败，就保持失败。
...
如果 `go stack e` 是 `Right True`，就变成 `Right False`；如果是 `Left err`，错误继续传播。

一句话：

> Functor 让我们只关心成功值怎么变，不用每次手写失败分支。

### 押题 4：Applicative 是什么？为什么 AND 用 `<$>` 和 `<*>`？

答：

Applicative 用来组合多个独立的上下文计算。  
AND 门求值中有：
...
如果不用 Applicative，就要写很多嵌套 `case`。  
这里用 Applicative 是因为 AND 的左右子表达式都要被求值，它们的错误处理模式是一样的。

### 押题 5：Monad 是什么？你们代码里哪里体现了 Monad？

答：

Monad 可以理解为：把带上下文的计算按顺序连接起来，而且后一步可以依赖前一步结果。
...
先解析第一个表达式，得到 `first`，后面的解析继续基于它构造左结合表达式。  
这就是后一步依赖前一步结果。
...
这里每一步都依赖前一步：先得到左 wire，再得到右 wire，再申请新 wire，再输出 netlist 行。  
`SimpleState` 把“当前编号”和“已经生成的行”自动传下去，这就是 State Monad 思想。

### 押题 6：Applicative 和 Monad 有什么区别？

答：

可以这样说：

> Applicative 适合组合相互独立的计算；Monad 适合后一步依赖前一步结果的计算。

在我们的代码里：

- `(&&) <$> go stack a <*> go stack b` 更像 Applicative，因为左右子表达式都要求值，然后合并结果。
- parser 的 `do` 和 netlist 的 `do` 更像 Monad，因为后续步骤要使用前一步解析出的表达式或生成出的 wire 名称。

### 押题 7：如果老师问 Monad laws 怎么办？

答：

可以诚实但专业地回答：

> 我们这个展示重点不是证明 Monad laws，而是展示 Monad 的工程用途。理论上，一个合法 Monad 应该满足 left identity、right identity 和 associativity。我们的 `SimpleState` 是标准 State 形式 `s -> (a, s)` 的最小实现，行为上遵循 State Monad 的组合方式。课堂 demo 里我们主要用它说明状态传递如何被封装。

### 押题 8：`do` 语法是不是特殊语法糖？

答：

是。`do` notation 本质上会被翻译成 `>>=` 和 `>>` 的组合。  
例如：
...
我们使用 `do` 是为了让 parser 和 netlist 生成过程更像顺序步骤，展示时更容易读。
